        ...
        <p className='text-center text-sm mb-6'>
          ...
          {state==='Sign Up' && (
            <div className='flex items-center mb-4 gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
              ...
              value={name} className='bg-transparent outline-none text-white' type="text" placeholder='Full Name' required />
            </div>)}
            ...
            <button className='rounded-full w-full py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-900 text-white font-medium cursor-pointer'>
              {state}
            </button>
          </form>

          {state==='Sign Up' ? (<p className='text-gray-400 text-center text-xs mt-4 cursor-pointer'>
            Already have an account?{' '}
            <span onClick={() => setState('Login')} className='text-blue-400 cursor-pointer underline'>Login Here</span>
          </p>) 
          : 
          (<p className='text-gray-400 text-center text-xs mt-4 cursor-pointer'>
